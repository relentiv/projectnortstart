...
export function DataTable<T>({
  ...
  return (
    <div className={cn("rounded-md border border-[#E5E5E3] bg-white overflow-hidden", className)}>
          ...
          {loading ? (
              ...
              <tr key={ri} className="border-b border-[#F2F2F0] last:border-0">
                ...
                {columns.map((c, ci) => (
                  <td key={ci} className={cn("px-4 py-4", c.className)}>
                    <span className="block h-3 rounded-sm bg-[#F2F2F0] animate-pulse" style={{ width: `${40 + ((ri + ci) % 4) * 12}%` }} />
                  </td>
                ))}
              </tr>
            ))
            ...
            data.map((row, ri) => {
              ...
              >
                ...
                {columns.map((col) => (
                  <td key={col.key} className={cn("px-4 py-4 text-[#0A0A0A]", col.align === "right" && "text-right", col.className)}>
